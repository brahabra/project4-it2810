...
export default function DisplaySearches(props: Props) {
  ...
  function showSearches() {
    return data?.searches.map(
      ({ title, created }: { title: string; created: string }) => (
        <View
					style={styles.searchesContainer}
          key={created}
        >
          <Octicons
        		...
        		size={15}
       			color="white"
						 ...
						 />
          <Text style={styles.searchesText}>{title}{" "}{created.slice(0, 10)}</Text>
        </View>
      )
    );
  }
  ...
  return (
    <View style={styles.displaySearchesContainer}>
      <Text >Showing the last {PAGE_OPTIONS.SEARCHES_SIZE} searches</Text>
